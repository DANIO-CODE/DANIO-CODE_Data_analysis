...
```{r adult_dopes_vs_lp_enhancers}

library(plyranges)

peaks_lp <- modelMatrices$longPec$peaks
names(peaks_lp) <- as.character(peaks_lp)
names(segments$LongPec) <- as.character(segments$LongPec)
segments$LongPec$pVals <- peaks_lp[names(segments$LongPec)]$pValue

adult_dopes_lp <- subsetByOverlaps(segments$LongPec, 
                                   dopes[dopes$type == "adult_marked"])
range_adult_dopes_lp <- quantile(adult_dopes_lp$pVals, probs = c(0, .9))

enhancer_dope_ranges_lp <- segments$LongPec %>% 
  filter(name == "5_EnhA1", 
         pVals >= range_adult_dopes_lp[1], 
         pVals <= range_adult_dopes_lp[2])

primed_dope_ranges_lp <- segments$LongPec %>% 
  filter(name == "7_EnhWk1", 
         pVals >= range_adult_dopes_lp[1], 
         pVals <= range_adult_dopes_lp[2])

set.seed(3141593)
subset_enhancer_dope_ranges_lp <- enhancer_dope_ranges_lp[
  sample(1:length(enhancer_dope_ranges_lp), size = 1500, replace = F)
] %>% arrange(pVals) %>% rev()

set.seed(3141593)
subset_primed_dope_ranges_lp <- primed_dope_ranges_lp[
  sample(1:length(primed_dope_ranges_lp), size = 1500, replace = F)
] %>% arrange(pVals) %>% rev()


comparison <- bind_ranges(dopes = adult_dopes_lp,
                          primed = subset_primed_dope_ranges_lp,
                          enhancers = subset_enhancer_dope_ranges_lp,
                          .id = "what") 
seqlevels(comparison) <- seqlevels(BSgenome.Drerio.UCSC.danRer10)
seqlengths(comparison) <- seqlengths(BSgenome.Drerio.UCSC.danRer10)
seqinfo(comparison) <- seqinfo(BSgenome.Drerio.UCSC.danRer10)


targetFiles <- fishPlotInputs$longPec$assayFiles
targetFiles$H3K27ac <- "/mnt/storage/Damir/new_samples_nov2020/K27ac_48hpf_sig/signal/macs2/rep1/2F_S1_R1_001.PE2SE.nodup.pval.signal.bw"
targetFiles$H3K4me1 <- "/mnt/storage/Damir/new_samples_nov2020/K4me1_48hpf_sig/signal/macs2/rep1/4F_S2_R1_001.PE2SE.nodup.pval.signal.bw"

lp_targets <- lapply(targetFiles, import)
lp_targets <- lapply(lp_targets, function(x){
  seqlevels(x) <- seqlevels(BSgenome.Drerio.UCSC.danRer10)
  seqlengths(x) <- seqlengths(BSgenome.Drerio.UCSC.danRer10)
  seqinfo(x) <- seqinfo(BSgenome.Drerio.UCSC.danRer10)
  x
})

comparisons_calc <- lapply(names(lp_targets), function(x){
  binnedAverage(comparison, 
                coverage(lp_targets[[x]], weight = "score"), 
                varname = x)
}) 

names(comparisons_calc) <- names(lp_targets)


foo_dist <- lapply(comparisons_calc, function(x){
  mcols(x)[,ncol(mcols(x))]}) %>% bind_cols() %>% 
  mutate(what = as.vector(comparison$what)) %>%
  mutate(across(-what, function(x) {
    (x - min(x)) / (max(x) - min(x))}))# %>%
  # mutate(atac_div_k4me3 = ATAC / H3K4me3, 
  #           atac_div_k27ac = ATAC / H3K27ac,
  #           atac_div_k4me1 = ATAC / H3K4me1) %>% 
  # group_by(what) %>% summarise(across(everything(), mean))

lp_SML <-  ScoreMatrixList(lp_targets,
                  resize(comparison, width = 4000, fix = "center"),
                  # strand.aware = T,
                  #bin.num = 4000,
                  weight.col = "score")

lp_SML <- intersectScoreMatrixList(lp_SML)

multiHeatMatrix(lp_SML, xcoords = c(-2000, 2000), 
                col = blues9, 
                group = factor(comparison$what[
                  as.numeric(rownames(lp_SML[[1]]))
                ]), winsorize = c(0, 95))
...
exprTC <- read_tsv("data/consens.meanTpmPerStage.canonicalEntireConsCl.df") %>%
  makeGRangesFromDataFrame(keep.extra.columns = T)

dope_promoters_expr_matrix <- log2(as.matrix(mcols(subsetByOverlaps(
  exprTC, dopes))[,3:18]) + .01)

dope_prom_ensg <- somTC[as.character(subsetByOverlaps(exprTC, dopes)$id)]$EnsG

set.seed(3141593)
dope_promoters_expr_matrix_km <- kmeans(dope_promoters_expr_matrix, 
                                        centers = 2)$cluster

ComplexHeatmap::Heatmap(dope_promoters_expr_matrix, 
  name = "log2(tpm)", cluster_columns = F, 
  row_split = dope_promoters_expr_matrix_km)

dope_prom_df <- tibble(location = as.character(somTC[as.character(
  subsetByOverlaps(exprTC, dopes)$id)]),
  ENSEMBL = sub(pattern = "\\.\\d+$", replacement = "", 
                                       dope_prom_ensg),
                       class = c("maternal", "zygotic")[dope_promoters_expr_matrix_km])

dope_prom_df <- bind_cols(dope_prom_df,
                          AnnotationDbi::select(org.Dr.eg.db, keys = 
...
```{r conserved_H3K27ac_ensembles}

ensambles_bins <- read_tsv("h3k27ac_tads_classes.bed", 
                           col_names = c("seqnames", "start", "end", "organism",
                                         "score", "stand", "thisckStart", 
                                         "thickEnd", "colour") ) %>% 
  makeGRangesFromDataFrame(keep.extra.columns = T)
ensemble_conserved_bins <- ensambles_bins[ensambles_bins$organism == "both" ]

grb_ensembles <- subsetByOverlaps(domeEnsamble, grb_tads)
conserved_proportion <- countOverlaps(grb_ensembles, ensemble_conserved_bins) / 
  countOverlaps(grb_ensembles, ensambles_bins)

conserved_ensambles <- grb_ensembles[conserved_proportion >= .5]

genes_in_conserved_ensembles <- subsetByOverlaps(somTC, 
                                                 conserved_ensambles + 12500)

genes_in_conserved_ensembles_ensg <- genes_in_conserved_ensembles %>% 
  as_tibble() %>% select(EnsG) %>% drop_na() %>% unique() %>%
  mutate(EnsG = sub("\\.\\d+$", "", EnsG)) %>% pull()

devel_universe <- somTC %>% 
  as_tibble() %>% select(EnsG) %>% drop_na() %>% unique() %>%
  mutate(EnsG = sub("\\.\\d+$", "", EnsG)) %>% pull()

devel_GO <- enrichGO(genes_in_conserved_ensembles_ensg, org.Dr.eg.db,
                     ont = "ALL", keyType = "ENSEMBL",
                     universe = devel_universe)

grb_universe <- somTC %>% subsetByOverlaps(grb_tads) %>%
  as_tibble() %>% select(EnsG) %>% drop_na() %>% unique() %>%
  mutate(EnsG = sub("\\.\\d+$", "", EnsG)) %>% pull()

grb_GO <- enrichGO(genes_in_conserved_ensembles_ensg, org.Dr.eg.db,
                     ont = "CC", keyType = "ENSEMBL",
                     universe = grb_universe)

conserved_ensembles_df <- AnnotationDbi::select(org.Dr.eg.db, 
                                 keys = genes_in_conserved_ensembles_ensg,
                                 columns = c("GENENAME", "SYMBOL"), 
                                 keytype = "ENSEMBL")

all_genes_GO <-  enrichGO(genes_in_conserved_ensembles_ensg, org.Dr.eg.db,
                     ont = "ALL", keyType = "ENSEMBL")
```
...
exons <- GenomicRanges::reduce(exons(TxDb.Drerio.UCSC.danRer10.ensGene)) %>%
  binnedAverage(numvar = phastCons_fish, varname = "phastCons") %>%
  mutate(stage = "Prim-5", name = "11_Exon", type = "exon") %>%
  as_tibble() %>% select(stage, name, phastCons, type)

set.seed(3141593)  
intergenic_cons <- genomeSegmentsSplit$Prim5$`10_Quies` %>%
  filter_by_non_overlaps(GenomicRanges::reduce(c(GenomicRanges::reduce(
    conservation$Prim5), 
    GenomicRanges::reduce(exons(TxDb.Drerio.UCSC.danRer10.ensGene))))) %>%
  resize(width = 500, fix = "center") %T>%
  {
    seqlevels(.) <- seqlevels(BSgenome.Drerio.UCSC.danRer10)
    seqlengths(.) <- seqlengths(BSgenome.Drerio.UCSC.danRer10)
    seqinfo(.) <- seqinfo(BSgenome.Drerio.UCSC.danRer10)
  } %>%
  # slice(sample(1:length(.), 100, replace = F)) %>%
  binnedAverage(numvar = phastCons_fish, varname = "phastCons") %>%
  mutate(stage = "Prim-5", name = "12_Intergenic", type = "Intergenic") %>%
  as_tibble() %>% select(stage, name, phastCons, type)
...
chromHMM_phastcons <- conservation %>% names() %>% lapply(function(x){
  ...
  filter(stage == "Prim5") %>%
  mutate(type = typ[name]) %>%
  rbind(exons, intergenic_cons) %>%
  mutate(name = factor(name, levels = stringr::str_sort(unique(name), 
                                                              numeric = T))) %>%
